...
run_task <- function(current_task,
                     ...
                     log_message = str_glue("{current_task} начинается, запускаю умение {skill}")){
  ...
  if(exists("current_task")){
    logger::log_debug("🦦  параметр `current_task` есть")  
  } else {
    logger::log_error("🦦  не заполнен параметр `current_task`")
    stop()
  }
  ...
  if(exists(skill)){
    ...
    logger::log_info("🦦  Меняю статус задачи {current_task} на `ignore`.")
    
    path_to_tasks |> 
      readr::read_csv(show_col_types = FALSE,
                      progress = FALSE) |> 
      dplyr::mutate(ignore = if_else(id == task_id, "ignore", ignore)) |> 
      readr::write_csv(file = path_to_tasks, na = "")
    
    if(curl::has_internet()){
      sent_gmail_message(subject = "Нет умения для задачи",
                         message = str_glue("Я не нашел умения {skill} для задачи {current_task} и поменял ее статус на `ignore`."),
